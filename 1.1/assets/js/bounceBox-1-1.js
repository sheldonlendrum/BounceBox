var bounceBox = new Class({
	...
	setPosition: function() {
		if(this.options.position=='center') {
			return window.getScroll().y + Math.ceil((window.getHeight() / 2) - ($(this.options.containerId).getHeight() /2)) +'px';
		} else {
			return window.getScroll().y + Math.ceil(this.options.position) +'px';
		}
	},
